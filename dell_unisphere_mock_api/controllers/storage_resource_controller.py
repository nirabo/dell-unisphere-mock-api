...
class StorageResourceController:
    ...
    async def create_storage_resource(self, request: Request, resource_data: StorageResourceCreate) -> dict:
        try:
            ...
            resource_response = StorageResourceResponse.model_validate(resource)

            formatter = UnityResponseFormatter(request)
            return formatter.format_response(
                content=resource_response, resource_type="storage_resource", resource_id=resource_response.id
            )
    ...
    async def get_storage_resource(self, request: Request, resource_id: str) -> dict:
        ...
        formatter = UnityResponseFormatter(request)
        return formatter.format_response(
            content=resource_response, resource_type="storage_resource", resource_id=resource_id
        )
    ...
    async def list_storage_resources(self, request: Request, resource_type: Optional[str] = None) -> List[dict]:
        ...
        resource_responses = [StorageResourceResponse.model_validate(r) for r in resources]

        formatter = UnityResponseFormatter(request)
        return formatter.format_collection(
            data=resource_responses,
            entry_links={i: [{"rel": "self", "href": f"/{r.id}"}] for i, r in enumerate(resource_responses)},
        )
    ...
    async def delete_storage_resource(self, request: Request, resource_id: str) -> None:
        # Get current resource
        resource = self.storage_resource_model.get_storage_resource(resource_id)
        if not resource:
            raise HTTPException(status_code=404, detail="Storage resource not found")
    ...
    async def remove_host_access(self, request: Request, resource_id: str, host_id: str) -> dict:
        ...
        resource_response = StorageResourceResponse.model_validate(resource.entries[0].content)
        formatter = UnityResponseFormatter(request)
        return formatter.format_response(
            content=resource_response, resource_type="storage_resource", resource_id=resource_id
        )
