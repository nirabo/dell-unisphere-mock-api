...
@router.post("/types/diskGroup/instances", response_model=DiskGroup, status_code=status.HTTP_201_CREATED)
async def create_disk_group(disk_group: DiskGroupCreate, current_user: dict = Depends(get_current_user)):
    ...
    if not disk_group_model.validate_raid_config(
        disk_group.raid_type,
        disk_group.stripe_width,
        len(disk_group.disk_ids)
    ):
        raise HTTPException(
            status_code=400,
