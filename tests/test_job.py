...
def test_get_job(sample_job_data, auth_headers):
    ...
    response = client.get(f"/api/instances/job/{job_id}", headers=headers, cookies=cookies)
...
def test_delete_job(sample_job_data, auth_headers):
    ...
    response = client.delete(f"/api/instances/job/{job_id}", headers=headers, cookies=cookies)
    assert response.status_code == 204

    get_response = client.get(f"/api/instances/job/{job_id}", headers=headers, cookies=cookies)
